...
public class ProductServiceImpl implements ProductService {
	...
	/*
	 * 상품 저장 - 화면에서 전달받은 정보를 저장하고 있는 ProductDTO를 Category Entity로 변환해서 영속 영역에 저장한다.
	 */
	@Override
...
//    }
	...
	/*
	 * 복잡한 검색시 사용시 필요 private BooleanBuilder getSearch(PageRequestDTO requestDTO){
	 * 
	 * String type = requestDTO.getType(); BooleanBuilder booleanBuilder = new
	 * BooleanBuilder(); QGuestbook qGuestbook = QGuestbook.guestbook; String
	 * keyword = requestDTO.getKeyword(); BooleanExpression expression =
	 * qGuestbook.categoryId.gt(0L); // gno > 0 조건만 생성
	 * booleanBuilder.and(expression); if(type == null || type.trim().length() ==
	 * 0){ //검색 조건이 없는 경우 return booleanBuilder; }
	 * 
	 * //검색 조건을 작성하기 BooleanBuilder conditionBuilder = new BooleanBuilder();
	 * if(type.contains("t")){
	 ...
	 * 
	 * //모든 조건 통합 booleanBuilder.and(conditionBuilder); return booleanBuilder; }
	 */
