...
//	}
	...
	//@Disabled
	@Commit	// 기본은 롤백
	public void testProductSaveBuilder() {
		...
		IntStream.rangeClosed(101, 200).forEach(i -> {
			Product product = Product.builder()
					.productName("Product Builder" + i)
					.price(150000)
					.category(category)
					.description("builder 상품설명" + i)
	...
	public void testGetProductById2() {
		Optional<Product> optProduct = productRepository.findById(10); // 데이터베이스 확인
	 ...
	 * 
	 * [Fetch 전략 - Eager / Lazy]
	 * - Fetch 전략을 Lazy로 하게 되면 지연 로딩이 되어 처음에는
	 *   상품 테이블로만 조회하고 카테고리 테이블이 필요한 시점 즉,
	 *   product.toString()이 되는 시점에
	 *   카테고리 테이블에 조회해서 카테고리 관련 모든 정보를 조회한다.
	 * - Fetch 전략을 안주거나, Eager로 하게 되면 처음부터 상품
	 *   테이블과 카테고리 테이블을 조인걸게 된다. 그렇게 되면
	 *   데이터베이스에 성능에 좋지 않다. 가급적이면 지연로딩을
	 *   사용하는게 낫다.	
	 * - @ManyToOne : 기본이 즉시로딩(Eager)
	 * - @OneToMay  : 기본이 지연로딩(Lazy)
	 */
	//@Test
	//@Disabled
	public void testGetProductById() {
		...
		//	  이 시점까지는 상품 테이블만 조회함.
		// 2. 아래의 product.toString()으로 상품 정보의 모든
		//	  내용(Category포함)을 보겠다고 하는 시점에 그제서야
		//	  카테고리 테이블에 조회를 한다.
		log.info("Product의 전체 정보 조회 : " + product.toString());
	}	
	/*
	 * 상품과 커테고리 정보를 동시에 조회
	 * 즉시 로딩 테스트
	...
	public void testProductUpdate() {
		...
		Product product = new Product();
		product.setProductId(100);	// 실존하는 상품명으로
